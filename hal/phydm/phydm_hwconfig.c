...
{							
	...
	if(isCCKrate)
	{
		...
		if (pDM_Odm->SupportICType & (ODM_RTL8703B)) {
			...
			#endif
		} else if (pDM_Odm->SupportICType & (ODM_RTL8188E | ODM_RTL8192E | ODM_RTL8723B)) /*3 bit LNA*/
		{
	...
	{		
		...
		{
			if (IS_HARDWARE_TYPE_8723B(pDM_Odm->Adapter))
				pPhyInfo->SignalStrength = (u1Byte) (PWDB_ALL - 4);
			else if (IS_HARDWARE_TYPE_8188E(pDM_Odm->Adapter))
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
			/* 2012/01/12 MH Use customeris signal strength from HalComRxdDesc.c/*/	
			pPhyInfo->SignalStrength = (u1Byte)(SignalScaleMapping(pDM_Odm->Adapter, avg_rssi));
#else
