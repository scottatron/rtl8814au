...
// 2009.10.15 by tynli.
static void SetFwRsvdPagePkt_8814(PADAPTER padapter, BOOLEAN bDLFinished)
{
	...
	u8	RsvdPageLoc[5] = {0};
	u16	BufIndex=0, PageSize = PAGE_SIZE_TX_8814;
	...
	RsvdPageNum = GetTxBufferRsvdPageNum8814(padapter, _FALSE);
	...
	else
	{
		...
		rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], PSPollLength, _TRUE, _FALSE, _FALSE);

		SET_8814A_H2CCMD_RSVDPAGE_LOC_PSPOLL(RsvdPageLoc, TotalPageNum);
		...
		ConstructNullFunctionData(
			...
			_FALSE, 0, 0, _FALSE);
		rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], NullFunctionDataLength, _FALSE, _FALSE, _FALSE);

		SET_8814A_H2CCMD_RSVDPAGE_LOC_NULL_DATA(RsvdPageLoc, TotalPageNum);
		...
		ConstructNullFunctionData(
			...
			get_my_bssid(&pmlmeinfo->network),
			_TRUE, 0, 0, _FALSE);
		rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], QosNullLength, _FALSE, _FALSE, _FALSE);

		SET_8814A_H2CCMD_RSVDPAGE_LOC_QOS_NULL_DATA(RsvdPageLoc, TotalPageNum);
	...
	if(!bDLFinished)
	{
		...
		FillH2CCmd_8814(padapter, H2C_RSVD_PAGE, 5, RsvdPageLoc);
	}
...
static void rtl8814_set_FwScanOffloadInfo_cmd(PADAPTER padapter, PRSVDPAGE_LOC rsvdpageloc, u8 enable)
{
	...
	FillH2CCmd_8814(padapter, H2C_D0_SCAN_OFFLOAD_INFO, H2C_SCAN_OFFLOAD_CTRL_LEN, u1H2CScanOffloadInfoParm);
...
static void rtl8814_set_ap_wow_rsvdpage_cmd(PADAPTER padapter,
		PRSVDPAGE_LOC rsvdpageloc)
{
	...
	FillH2CCmd_8814(padapter, H2C_BCN_RSVDPAGE,
	...
	FillH2CCmd_8814(padapter, H2C_8192E_PROBERSP_RSVDPAGE,
...
//	to Hw again and set the lengh in descriptor to the real beacon lengh.
// 2009.10.15 by tynli.
static void rtl8814_set_AP_FwRsvdPagePkt(PADAPTER padapter,
		BOOLEAN bDLFinished)
{
	PHAL_DATA_TYPE pHalData;
	struct xmit_frame	*pcmdframe;
	...
	DBG_871X("%s: Set RSVD page location to Fw ,TotalPacketLen(%d), TotalPageNum(%d)\n", __FUNCTION__,TotalPacketLen,TotalPageNum);
	rtl8814_set_ap_wow_rsvdpage_cmd(padapter, &RsvdPageLoc);
...
//for wake on wlan
static void rtl8814_set_FwRsvdPagePkt(PADAPTER padapter, BOOLEAN bDLFinished)
{
	PHAL_DATA_TYPE pHalData;
	struct xmit_frame	*pcmdframe;	
	struct pkt_attrib	*pattrib;
	...
	ConstructNullFunctionData(
		...
		get_my_bssid(&pmlmeinfo->network),
		_FALSE, 0, 0, _FALSE);
	rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], NullDataLength, _FALSE, _FALSE,  _FALSE);
	...
	ConstructProbeRsp(
		...
		get_my_bssid(&pmlmeinfo->network),
		_FALSE);
	rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], ProbeRspLength, _FALSE, _FALSE);
	...
	rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], ARPLegnth, _FALSE, _FALSE, _TRUE);
	...
	rtl8814a_fill_fake_txdesc(padapter, &ReservedPagePacket[BufIndex-TxDescLen], GTKLegnth, _FALSE, _FALSE, _TRUE);
#if 0
	{
		int gj;
		printk("123GTK pkt=> \n");
...
#endif //CONFIG_WOWLAN
	{
		...
		if (pwrctl->pno_in_resume == _FALSE) {
			...
			rtl8814a_fill_fake_txdesc(padapter,
				&ReservedPagePacket[BufIndex-TxDescLen],
	...
	if(check_fwstate(pmlmepriv, _FW_LINKED)) {
		...
		if(pwrctl->pno_in_resume)
			rtl8814_set_FwScanOffloadInfo_cmd(padapter,
					&RsvdPageLoc, 0);
		else
			rtl8814_set_FwScanOffloadInfo_cmd(padapter,
					&RsvdPageLoc, 1);
...
static void rtl8814_set_FwRemoteWakeCtrl_Cmd(PADAPTER padapter, u8 benable)
{
...
exit:
	...
	FillH2CCmd_8814(padapter, H2C_REMOTE_WAKE_CTRL,
...
void rtl8814_set_FwMediaStatusRpt_cmd(PADAPTER	padapter, u8 mstatus, u8 macid)
{
	...
	FillH2CCmd_8814(padapter, H2C_MEDIA_STATUS_RPT, H2C_MEDIA_STATUS_RPT_LEN, u1H2CMediaStatusRptParm);
}

static void rtl8814_set_FwDisconDecision_cmd(PADAPTER padapter, u8 benable)
{
	...
	FillH2CCmd_8814(padapter, H2C_DISCON_DECISION, H2C_DISCON_DECISION_LEN, u1H2CDisconDecisionParm);
}

static void rtl8814_set_FwKeepAlive_cmd(PADAPTER padapter, u8 benable, u8 pkt_type)
{
	...
	FillH2CCmd_8814(padapter, H2C_KEEP_ALIVE, H2C_KEEP_ALIVE_CTRL_LEN, u1H2CKeepAliveParm);
}

static void rtl8814_set_FwWoWlanCtrl_Cmd(PADAPTER padapter, u8 bFuncEn)
{
	...
	FillH2CCmd_8814(padapter, H2C_WOWLAN, H2C_WOWLAN_LEN, u1H2CWoWlanCtrlParm);
}

static void rtl8814_set_FwAOACGlobalInfo_Cmd(PADAPTER padapter,  u8 group_alg, u8 pairwise_alg)
{
	...
	FillH2CCmd_8814(padapter, H2C_AOAC_GLOBAL_INFO, H2C_AOAC_GLOBAL_INFO_LEN, u1H2CAOACGlobalInfoParm);
}
	...
	if(mstatus == RT_MEDIA_CONNECT)
	{
		...
		do
		{
			...
			if (pwrpriv->wowlan_ap_mode)
				rtl8814_set_AP_FwRsvdPagePkt(padapter, 0);
			else
				rtl8814_set_FwRsvdPagePkt(padapter, 0);
#else
			// download rsvd page.
			rtl8814_set_FwRsvdPagePkt(padapter, 0);
#endif
...
static void rtl8814_set_FwWoWlanRelated_cmd(_adapter* padapter, u8 enable)
{
	struct security_priv *psecpriv = &padapter->securitypriv;
	struct pwrctrl_priv *ppwrpriv = adapter_to_pwrctl(padapter);
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
...
_func_enter_;
	if(enable)
	{
		rtl8814_set_FwAOACGlobalInfo_Cmd(padapter, psecpriv->dot118021XGrpPrivacy, psecpriv->dot11PrivacyAlgrthm);
		//rtw_hal_set_global_info_cmd(padapter, psecpriv->dot118021XGrpPrivacy, psecpriv->dot11PrivacyAlgrthm);
		rtl8814_download_rsvd_page(padapter, RT_MEDIA_CONNECT);	//RT_MEDIA_CONNECT will confuse in the future
		...
		if(!(ppwrpriv->wowlan_pno_enable))
		{
			...
			if (psta != NULL)
				rtl8814_set_FwMediaStatusRpt_cmd(padapter, RT_MEDIA_CONNECT, psta->mac_id);
		}	
		else
		...
		if(!(ppwrpriv->wowlan_pno_enable)) {
			rtl8814_set_FwDisconDecision_cmd(padapter, enable);
			//rtw_hal_set_disconnect_decision_cmd(padapter, enable);
			rtw_msleep_os(2);

			if ((psecpriv->dot11PrivacyAlgrthm != _WEP40_) || (psecpriv->dot11PrivacyAlgrthm != _WEP104_))
				pkt_type = 1;
			rtl8814_set_FwKeepAlive_cmd(padapter, enable, pkt_type);
		...
		rtl8814_set_FwWoWlanCtrl_Cmd(padapter, enable);
...
#endif
		rtl8814_set_FwRemoteWakeCtrl_Cmd(padapter, enable);
		//rtw_hal_set_remote_wake_ctrl_cmd(padapter, enable);
		rtw_msleep_os(2);
		rtl8814_set_FwWoWlanCtrl_Cmd(padapter, enable);
		//rtw_hal_set_wowlan_ctrl_cmd(padapter, enable);
...
{
	rtl8814_set_FwWoWlanRelated_cmd(padapter, enable);
}
#endif //CONFIG_WOWLAN
...
//			MPTBT_FwC2hBtMpCtrl(Adapter, tmpBuf, c2hCmdLen);
		...
		case C2H_8814A_RA_PARA_RPT:
			C2HRaReportHandler_8814(Adapter, tmpBuf, c2hCmdLen);
			break;
