...
u16
GetTxBufferRsvdPageNum8814A(
	IN	PADAPTER	Adapter,
	IN	BOOLEAN		bWoWLANBoundary
)
{
	u16			RsvdPageNum=0;
	u16			TxPageBndy= LAST_ENTRY_OF_TX_PKT_BUFFER_8814A; 

	if(bWoWLANBoundary)
	{
		rtw_hal_get_def_var(Adapter, HAL_DEF_TX_PAGE_BOUNDARY_WOWLAN, (u16*)&TxPageBndy);
	}
	else
	{
	...
	)
{
	...
	} while ((counter++ < 500) && (!(value32 &  CPU_DL_READY)));
...
)
{
	...
	switch(pFirmware->eFWSource)
	{
		case FW_SOURCE_IMG_FILE:
			#ifdef CONFIG_FILE_FWIMG
			rtStatus = rtw_retrive_from_file(rtw_fw_file_path, FwBuffer8812, FW_SIZE);
...
{	
	...
	for(rfPath = 0 ; rfPath < MAX_RF_PATH ; rfPath++)
	{
		...
		{
			hal_GetChnlGroup8814A(channel5G[ch], &group);
		...
		for(ch = 0 ; ch < CHANNEL_MAX_NUMBER_5G_80M; ch++) 
		{
			...
			hal_GetChnlGroup8814A(channel5G_80M[ch], &group);
	...
	)
{
	...
	if (!AutoloadFail) {
		...
		} else if (trx_antenna_option == 0x6e) {
			trx_antenna = RF_2T4R;
	...
	)
{
	...
	if(!AutoLoadFail)
	{
		...
		if(pHalData->CrystalCap == 0xFF)
			pHalData->CrystalCap = EEPROM_Default_CrystalCap_8814;	 //what value should 8812 set?
	}
	else
	{
	...
	)
{
	...
	switch(pHalData->RFEType)
	{
		...
		case 2:	//  2G/5G LNA/PA
			...
			break;
		default:	case 0:	//Internal LNA/PA
			break;
	}
...
VOID
hal_ReadAmplifierType_Jaguar(
	IN	PADAPTER	Adapter,
	IN	u8*		PROMContent,
	IN	BOOLEAN		AutoloadFail
	)
{
	u8			PAType = 0, LNAType = 0;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);

	u8 extTypePA_2G_A  = (PROMContent[0xBD] & BIT2)      >> 2; // 0xBD[2]
	u8 extTypePA_2G_B  = (PROMContent[0xBD] & BIT6)      >> 6; // 0xBD[6]
	u8 extTypePA_2G_C  = (PROMContent[0xBE] & BIT2)      >> 2; // 0xBE[2]
	u8 extTypePA_2G_D  = (PROMContent[0xBE] & BIT6)      >> 6; // 0xBE[6]

	u8 extTypePA_5G_A  = (PROMContent[0xBF] & BIT2)      >> 2; // 0xBF[2]
	u8 extTypePA_5G_B  = (PROMContent[0xBF] & BIT6)      >> 6; // 0xBF[6]
	u8 extTypePA_5G_C  = (PROMContent[0xC0] & BIT2)      >> 2; // 0xC0[2]
	u8 extTypePA_5G_D  = (PROMContent[0xC0] & BIT6)      >> 6; // 0xC0[6]

	u8 extTypeLNA_2G_A = (PROMContent[0xBD] & (BIT1|BIT0)) >> 0; // 0xBD[1:0]
	u8 extTypeLNA_2G_B = (PROMContent[0xBD] & (BIT5|BIT4)) >> 4; // 0xBD[5:4]
	u8 extTypeLNA_2G_C = (PROMContent[0xBE] & (BIT1|BIT0)) >> 0; // 0xBE[1:0]
	u8 extTypeLNA_2G_D = (PROMContent[0xBE] & (BIT5|BIT4)) >> 4; // 0xBE[5:4]

	u8 extTypeLNA_5G_A = (PROMContent[0xBF] & (BIT1|BIT0)) >> 0; // 0xBF[1:0]
	u8 extTypeLNA_5G_B = (PROMContent[0xBF] & (BIT5|BIT4)) >> 4; // 0xBF[5:4]
	u8 extTypeLNA_5G_C = (PROMContent[0xC0] & (BIT1|BIT0)) >> 0; // 0xC0[1:0]
	u8 extTypeLNA_5G_D = (PROMContent[0xC0] & (BIT5|BIT4)) >> 4; // 0xC0[5:4]

	hal_ReadPAType_8814A(Adapter, PROMContent, pHalData->bautoload_fail_flag, &PAType, &LNAType);

	if ((PAType & (BIT5|BIT4)) == (BIT5|BIT4)) // [2.4G] Path A and B are both extPA 
	{
		pHalData->TypeGPA  = extTypePA_2G_B  << 2 | extTypePA_2G_A;

		if ((PAType & (BIT6|BIT7)) == (BIT6|BIT7)) // [2.4G] Path C and D are both extPA 
			pHalData->TypeGPA  |= (extTypePA_2G_C  << 4 | extTypePA_2G_D << 6);
	}
	
	if ((PAType & (BIT1|BIT0)) == (BIT1|BIT0)) // [5G] Path A and B are both extPA 
	{	
		pHalData->TypeAPA  = extTypePA_5G_B  << 2 | extTypePA_5G_A; 

		if ((PAType & (BIT2|BIT3)) == (BIT2|BIT3)) // [5G] Path C and D are both extPA 
			pHalData->TypeAPA  |= (extTypePA_5G_C  << 4 | extTypePA_5G_D << 6);
	}
	
	if ((LNAType & (BIT5|BIT4)) == (BIT5|BIT4)) // [2.4G] Path A and B are both extLNA
	{
		pHalData->TypeGLNA = extTypeLNA_2G_B << 2 | extTypeLNA_2G_A;
		
		if ((LNAType & (BIT7|BIT6)) == (BIT7|BIT6)) // [2.4G] Path C and D are both extLNA
			pHalData->TypeGLNA |= (extTypeLNA_2G_C << 4 | extTypeLNA_2G_D << 6);

	}
	if ((LNAType & (BIT1|BIT0)) == (BIT1|BIT0)) // [5G] Path A and B are both extLNA
	{
		pHalData->TypeALNA = extTypeLNA_5G_B << 2 | extTypeLNA_5G_A;
				
		if ((LNAType & (BIT3|BIT2)) == (BIT3|BIT2)) // [5G] Path C and D are both extLNA
			pHalData->TypeALNA |= (extTypeLNA_5G_C << 4 | extTypeLNA_5G_D << 6);
	}	
...
void InitPGData8814A(PADAPTER padapter)
{
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
	{
		...
		tmp = _halReadMACAddrFromFile(padapter, pHalData->EEPROMMACAddr);
		pHalData->bloadmac_fail_flag = ((tmp==_FAIL) ? _TRUE : _FALSE);

#ifdef CONFIG_SDIO_HCI
		//addr = EEPROM_MAC_ADDR_8821AS;
#elif defined(CONFIG_USB_HCI)
		addr = EEPROM_MAC_ADDR_8814AU;
...
void InitDefaultValue8814A(PADAPTER padapter)
{
	...
	pHalData->macid_num = MACID_NUM_8814A;
	pHalData->cam_entry_num = CAM_ENTRY_NUM_8814A;
...
u8 GetHalDefVar8814A(PADAPTER padapter, HAL_DEF_VARIABLE variable, void *pval)
{
	...
	switch (variable)
	{
		...
		case HAL_DEF_EXPLICIT_BEAMFORMER:
			DBG_871X("%s,%d, 8814 need to check \n", __FUNCTION__,__LINE__);
...
s32 c2h_id_filter_ccx_8814a(u8 *buf)
{
	...
	s32 ret = _FALSE;
	if (c2h_evt->id == C2H_8814A_TX_REPORT)
		ret = _TRUE;
...
void rtl8814_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->GetHalODMVarHandler = &rtl8814_GetHalODMVar;
	pHalFunc->SetHalODMVarHandler = &rtl8814_SetHalODMVar;
