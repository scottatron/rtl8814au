	...
	)
{
	...
	rtw_write16(Adapter, REG_RXFF_PTR_8814A, MAX_RX_DMA_BUFFER_SIZE_8814A-1); //yx_qi 20140331
	...
	)
{
	...
	if(_FALSE == AutoloadFail)  
	{
		...
		_rtw_memcpy(pHalData->EEPROMMACAddr, &PROMContent[EEPROM_MAC_ADDR_8814AU], ETH_ALEN);
	}
	else
	{
		//Random assigh MAC address
		u8	sMacAddr[ETH_ALEN] = {0x00, 0xE0, 0x4C, 0x88, 0x14, 0x00};
	...
	switch(pHalData->EEPROMCustomerID)
	{
		case EEPROM_CID_DEFAULT:
			if((pHalData->EEPROMVID == 0x2001) && (pHalData->EEPROMPID == 0x3308))
				...
				pHalData->CustomerID = RT_CID_DLINK;
			else if((pHalData->EEPROMVID == 0x2001) && (pHalData->EEPROMPID == 0x331a))
				pHalData->CustomerID = RT_CID_DLINK;
	...
	)
{
	...
	if (check_phy_efuse_macaddr_info_valid(Adapter) == _TRUE) {
		...
		Hal_GetPhyEfuseMACAddr(Adapter, pHalData->EEPROMMACAddr);
	} else {
...
#else //CONFIG_EFUSE_CONFIG_FILE
	hal_ReadMACAddress_8814AU(Adapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
#endif //CONFIG_EFUSE_CONFIG_FILE
...
VOID
hal_CustomizedBehavior_8814AUsb(
	IN PADAPTER 		Adapter
	)
{
#if 0
	...
	)
{
	...
	DBG_871X("====> ReadAdapterInfo8814AU\n");
	...
	//hal_CustomizedBehavior_8814AUsb(Adapter);
...
_func_enter_;
	...
	switch(variable)
	{
		...
		case HW_VAR_USB_MODE:
			if(*val == 1)
				rtw_write8(Adapter, REG_HCI_OPT_CTRL_8814A, 0x4);
			else if(*val == 2)
				rtw_write8(Adapter, REG_HCI_OPT_CTRL_8814A, 0x8);
			
			rtw_write8(Adapter, REG_SDIO_CTRL_8814A, 0x2);
